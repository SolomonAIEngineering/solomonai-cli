...
check_problematic_files() {
    local target_dir="$1"
    local -a problematic_files=("tsconfig.json.hbs" "package.json.hbs")
...
# Main execution
main() {
    log_info "Starting package generation process..."
    ...
    local template_dir="turbo/generator/templates/package"
    ...
    log_success "Package '$package_name' created successfully in $target_dir"
